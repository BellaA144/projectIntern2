...
export default function Page() {
  ...
  useEffect(() => {
    async function fetchData() {
      const { books, totalPages } = await getBooks({ ...filters, search: debouncedSearch });
      setBooks(books);
      
      if (filters.page > totalPages) {
        setFilters((prev) => ({ ...prev, page: Math.max(totalPages, 1) }));
      }
    ...
    fetchData();
  }, [debouncedSearch, filters.category, filters.page]);
